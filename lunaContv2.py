...
class Environment(Process):
    def __init__(self, env_idx, child_conn, env_name, state_size, action_size, visualize=False):
        ...
        self.discretizationer= Discretizationer(BINS_NUM,-1,1)       
    ...
    def run(self):
        ...
        while True:
            ...
            #idx to val
            action_cont=self.discretizationer.idx_to_val_vec(action)
            state, reward, done, info = self.env.step(action_cont)
...
class Actor_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        ...
        X = Dense(128, activation="tanh", kernel_initializer=tf.random_normal_initializer(stddev=0.01))(X_input)
        X = Dense(64, activation="tanh", kernel_initializer=tf.random_normal_initializer(stddev=0.01))(X)
...
class Critic_Model:
    def __init__(self, input_shape, action_space, lr, optimizer):
        ...
        V = Dense(132, activation="tanh", kernel_initializer='he_uniform')(X_input)
        V = Dense(64, activation="tanh", kernel_initializer='he_uniform')(V)
...
class Discretizationer:
    def __init__(self,bins_num,min,max,vec_size):
    ...
    def vec_val_to_idx(self,vec):
        ...
        for i in range(length):
            ...
            sum += (bin_vec[i] * j)
        return sum
    def idx_to_val_vec(self,idx):
        bin_vec=np.zeros(self.vec_size)
        for i in range(self.vec_size):
            j=np.power(self.bins_num,self.vec_size-i-1)
            bin_of=np.floor(idx/j)
            idx-=(bin_of*j)
            bin_vec[self.vec_size-i-1]=bin_of
        #print(bin_vec)
...
class PPOAgent:
    ...
    def __init__(self, env_name):
        ...
        self.action_size = self.discretizationer.bins_num ** 2
        ...
        self.Actor_name = f"{self.env_name}_bp_PPO_Actor.h5"
        self.Critic_name = f"{self.env_name}_bp_PPO_Critic.h5"
    ...
    def run(self): # train only when episode is finished
        ...
        while True:
            ...
            while not done:
                ...
                next_state, reward, done, _ = self.env.step(action_cont)
    ...
    def test(self, test_episodes = 100):
        ...
        for e in range(100):
            ...
            while not done:
                ...
                #idx to val
                action_cont=self.discretizationer.idx_to_val_vec(action)
                state, reward, done, _ = self.env.step(action_cont)
                reward=change_reward(reward)
                state = np.reshape(state, [1, self.state_size[0]])
                score += reward
                if done:
