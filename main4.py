...
class ActorCritic(RLModel):
    ...
    def calculate_lr(self):
        self.alpha=LR_DISCOUNT*self.alpha
        self.beta=LR_DISCOUNT*self.beta
        K.set_value(self.actor.optimizer.learning_rate,self.alpha )
        K.set_value(self.critic.optimizer.learning_rate,self.beta )
        logging.info(f'lr to {LR_DISCOUNT}*lr={self.alpha} ,{self.alpha}')
...
class AgentDQN(RLModel):
    ...
    def calculate_lr(self):
        ...
        print(f'lr to {LR_DISCOUNT} * lr={self.lr}')
...
class AgentDDQN(RLModel):
    ...
    def _choose_action_index(self, observation):
        ...
        if np.random.random() < 0.5:
            choose_nn1=True     
        if choose_nn1:
            q1 = self.q_eval1.predict(observation)
            argm = np.argmax(q1,axis=1)
            return argm[0]
        q2 = self.q_eval2.predict(observation)
    ...
    def train2nn(self, nn1, nn2, states, action_idx, rewards, states_next, non_terminal):
        action_values = np.array(self.action_space, dtype=np.int8)
        action_indices = np.dot(action_idx, action_values)
        ...
        batch_index = np.arange(self.batch_size, dtype=np.int32)
        temp=np.max(q_next, axis=1)
        q_target[batch_index, action_indices] = rewards + \
                    self.gamma*np.max(q_next, axis=1)*non_terminal
        
        _ = nn1.fit(states, q_target, verbose=0)
